...
BASE_JACKPOT = 60000 # Base jackpot diperbarui menjadi 60.000
lottery_players = []
lottery_channels = set()

# --- DATABASE SETUP (SQLite) ---
conn = sqlite3.connect('pemerintahan.db')
c = conn.cursor()
...
try: c.execute('ALTER TABLE warga ADD COLUMN last_daily TIMESTAMP')
except sqlite3.OperationalError: pass
try: c.execute('ALTER TABLE warga ADD COLUMN last_work TIMESTAMP')
except sqlite3.OperationalError: pass
try: c.execute('ALTER TABLE warga ADD COLUMN last_beg TIMESTAMP')
except sqlite3.OperationalError: pass
try: c.execute('ALTER TABLE warga ADD COLUMN last_steal TIMESTAMP')
except sqlite3.OperationalError: pass
conn.commit()

def get_user(user_id):
    c.execute("SELECT credits, last_hourly, last_weekly, last_daily, last_work, last_beg, last_steal FROM warga WHERE user_id = ?", (user_id,))
    result = c.fetchone()
    if result is None:
        c.execute("INSERT INTO warga (user_id, credits) VALUES (?, 0)", (user_id,))
        conn.commit()
        return (0, None, None, None, None, None, None)
    return result

def update_credits(user_id, amount):
    c.execute("UPDATE warga SET credits = credits + ? WHERE user_id = ?", (amount, user_id))
    conn.commit()
    
def update_cooldown(user_id, column):
    now = datetime.now().isoformat()
    c.execute(f"UPDATE warga SET {column} = ? WHERE user_id = ?", (now, user_id))
    conn.commit()

def check_cooldown(last_time_str, delta):
...
async def beg(interaction: discord.Interaction):
    ...
    reward = random.randint(500, 750) # Updated reward range
...
async def work(interaction: discord.Interaction):
    ...
    reward = random.randint(1500, 3000) # Updated reward range
    update_credits(user_id, reward)
...
async def hourly(interaction: discord.Interaction):
    ...
    reward = random.randint(5000, 7500) # Updated reward range
    update_credits(user_id, reward)
...
async def daily(interaction: discord.Interaction):
    ...
    reward = random.randint(30000, 50000) # Updated reward range
    update_credits(user_id, reward)
...
async def weekly(interaction: discord.Interaction):
    ...
    reward = random.randint(450000, 650000) # Updated reward range
    update_credits(user_id, reward)
...
bot.run('ODg5MTU3ODY0ODM1MzgzMzE3.GJhkq9.WqbT3UWhYbYW8wtN_fiSW9qKCIn8qVyubrJvgU')